...
export function generateRecommendationElements(recommendationList){
    ...
    recommendationList.forEach((question, index) => {
        ...
        newQuestion.innerHTML = `<div class="content">${question.split(" ").map((word, index) => `<span class="word-${index}">${word}</span>`).join(" ")}</div>
                                <button class="recommendation-speak">
